...
class GoPiGo3:
    ...
    def __init__(self):
        # Creates a node with name 'rubot_control' and make sure it is a
        ...
        self.f_goal = radians(float(rospy.get_param("~f")))
        print(self.f_goal)
    ...
    def update_odom(self, data):
        ...
        self.yaw = self.rpw[2]
        #rospy.loginfo("Position x= %1.2f m, y= %1.2f m at fi = %1.1f degrees.",self.x_pose,self.y_pose,degrees(self.yaw))
    ...
    def move2pose(self):
        ...
        while self.euclidean_distance(goal_odom) >= distance_tolerance:
            ...
            while abs(self.f_goal-self.yaw) >= angle_tolerance:
                ...
                # Publishing our vel_msg
                self.velocity_publisher.publish(vel_msg)
                rospy.loginfo("Orientation f: " + str(round(degrees(self.f_goal), ndigits=1)))
                rospy.loginfo("Orientation YAW: " + str(round(degrees(self.yaw), ndigits=1)))
                rospy.loginfo("Orientation error: " + str(round(self.f_goal-self.yaw, ndigits=2)))
                # Publish at the desired rate.
                self.rate.sleep()
